...
The launcher table (id, title, directory, host binary) is at the top of
`main()` in `src/main.cpp` — add a row per port. `src/recomp_input.h` is
the shared input-conventions header the game hosts use.
...
In the GUI: pick the game, **Settings → Import GDI image…**, choose the
...
status line reports progress; failures point at a log in
...
**Settings → Clear imported data** wipes a game's `disc/` folder (after a
confirmation) so a different dump can be imported. Play refuses to start
a game with no content imported.
